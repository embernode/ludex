...
5. **Gamescope handling**: if the candidate's exe or any ancestor resolves to `gamescope`/`gamescope-wl`, the gate is bypassed and the gamescope child is trusted as the running game. Graceful degradation rather than a first-class feature.
...
- **Tauri + WebKitGTK in sandboxed runtimes** is a known source of runtime-version-skew bugs. Native packaging (AUR) is the primary distribution path; sandboxed bundle formats are not roadmapped.
