...
Each recognised game has its sessions persisted to SQLite with two runtime figures: **full runtime** (wall-clock session duration) and **interactive runtime** (full runtime minus billable idle intervals, detected via the Wayland `ext-idle-notify` protocol's input-based notifications — games' idle inhibitors don't mask AFK time — with `logind.IdleHint` as the fallback off Wayland, and a configurable cutscene-grace window so non-skippable cutscenes and dialogue trees aren't read as "user stepped away").
